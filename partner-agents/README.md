...
- **Spawns partners into real terminal tabs** on Windows, macOS, and Linux — Windows Terminal, iTerm2, Terminal.app, GNOME Terminal, Konsole, WezTerm, kitty, Ghostty, foot, Rio, Alacritty, tmux, zellij and xterm are detected automatically, and **a terminal it has never heard of works too**: whatever hosts your session announces itself in the environment, gets probed for the flags that run a command, set a directory and set a title, and is driven from those — the same way an unknown agent CLI is. `terminals` shows what can open a tab and which wins, `--terminal <name>` forces one, a JSON entry in `~/.claude/partner-terminals.json` adds or corrects one, and a platform default (`cmd.exe`, `open -a Terminal`, `x-terminal-emulator`) means a spawn still opens a tab when nothing at all is recognised.
- **Native Orca support.** Running inside [Orca](https://orca.computer)? Partners open as Orca tabs in the current worktree, beside the session that spawned them — not in a detached OS window. It is one entry in the terminal table like any other: a host you are already inside wins, then the terminal on screen, then anything that can open a window.
...
- **Wakes partners from inside a sandbox.** Waking a partner means typing into its tab with the terminal's CLI, which a sandboxed agent (Codex in `workspace-write`) is not allowed to do. Its wake-ups are handed to a small per-session waker process instead. The waker is started by whichever agent can drive the terminal, and it checks that it can before taking the job. Every partner can wake every other, whichever one is sandboxed.
- **Notices when a partner's CLI is gone.** If a partner ignores three wake-ups in a row, its CLI has exited and a shell is reading them. The wake-ups stop, everyone is told, and `restart --id <id>` relaunches that one partner in a new tab, briefed from the transcript. The dead partner no longer blocks the idle pause.
- **One listener per agent, enforced.** Two `wait`s for the same agent split its inbox, and the old marker made that worse by looking like *nothing* was listening — so agents were told to start yet another. Ownership now carries a heartbeat: a duplicate idles instead of consuming and says so, a dead owner's place is taken automatically, and `read` tells an agent whether it already has one in flight.
...
- **Stops paying for silence, but only once the work is done.** An idle `wait` hands control back to its model every cycle, and each of those is a turn spent deciding to wait again. So once **every** partner has finished its work and a minute has gone by with nothing new, the loops stop. Every partner ends its turn, including the session you started them from, whether that is Claude Code or Codex. They all follow the same rule. "Finished" is checked per agent, not assumed from silence: it is back in `wait`, it owes nobody a reply, it is not mid-turn, and it has stayed that way for the full minute. If even one partner is still working, everybody keeps looping. Nothing is needed to resume: the first message to any partner (a `send`, a `claim`, or you writing to one of them) wakes the whole session and types a wake-up into every tab. It never pauses while a tab sits in a terminal that cannot be typed into, because a message could not wake that tab. The one exception is a session running outside a terminal like Orca, whose tab cannot be typed into. It keeps a background `wait` that sleeps through the pause for free. `/partner idle` shows who is still working, and `idle --after 300` changes the minute (`0` turns it off).
...
/partner idle                         # who is still working, and whether the loops will pause
/partner stop --all
```
...
Once every agent has finished its work and a minute passes with nothing new, the loops stop. Every agent ends its turn, `p1` included. The first message to any agent wakes them all by typing a wake-up into each tab, `p1`'s too.
...
A `UserPromptSubmit` hook marks the Claude Code session's turn as open until the `Stop` hook sees it end, so the others never pause while that session is mid-work, and your message to it wakes a paused session. A `Stop` hook (`hooks/hooks.json`) keeps any agent — the session that ran the skill included — from ending a turn while a message addressed to it is still unanswered in the transcript. It is what stops the non-blocking session agent from going silent the moment the human's attention moves to another tab.
