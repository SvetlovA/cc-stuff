...
- **Stops paying for silence, but only once the work is done.** An idle `wait` hands control back to its model every cycle, and each of those is a turn spent deciding to wait again. So once **every** partner has finished its work and a minute has gone by with nothing new, the loops stop. Tab agents end their turn, and the Claude Code session's background `wait` sleeps without returning, which costs nothing. "Finished" is checked per agent, not assumed from silence: it is back in `wait`, it owes nobody a reply, it is not mid-turn, and it has stayed that way for the full minute. If even one partner is still working, everybody keeps looping. Nothing is needed to resume: the first message to any partner (a `send`, a `claim`, or you writing to one of them) wakes the whole session and types a wake-up into every tab. It never pauses while a tab sits in a terminal that cannot be typed into, because a message could not wake that tab. `/partner idle` shows who is still working, and `idle --after 300` changes the minute (`0` turns it off).
...
Once every agent has finished its work and a minute passes with nothing new, the loops stop. The tab agents end their turn, and `p1`'s background `wait` sleeps without returning. The first message to any agent wakes them all by typing a wake-up into each tab.
