...
- **Stops paying for silence.** An idle `wait` hands control back to its model every cycle, and each of those is a turn spent deciding to wait again. When nothing has happened for 5 minutes *and every agent is sitting in `wait`*, the session pauses: tab agents stop looping, and the Claude Code session's background `wait` sleeps without returning, which costs nothing. If even one agent is still working, everybody keeps looping. Nothing is needed to undo it: any `send`, `claim`, `kickoff` or `spawn` resumes the session and types a wake-up into every tab, or run `/partner wake`. It never pauses on its own while a tab sits in a terminal that cannot be typed into, because nothing could wake that tab again. `/partner pause --after 900` changes the threshold, and `--after 0` turns it off.
...
/partner pause                        # stop every loop now (idle sessions pause on their own)
/partner wake                         # resume a paused session and wake every tab
/partner stop --all
```
...
When the transcript has been quiet for 5 minutes and every agent is in `wait`, the loops stop until there is work again. The tab agents end their turn, `p1`'s background `wait` sleeps without returning, and the first `send`, `claim`, `spawn` or `wake` types a wake-up into each tab.
