...
**The session that started the partners keeps its `wait` running while the partners are paused.** Before 0.21.0, whichever agent ran `init` (Claude Code or Codex) kept a sleeping `wait` through every pause. For a foreground Codex `wait`, the tool cap killed it and Codex re-armed it every cycle. From 0.21.0 it records its own tab and stops like every other agent. If it still keeps a `wait`, `wait` prints that it is *sleeping*, which means its terminal gives no `self_handle`. That is the case outside Orca. Add `self_handle` to the terminal's entry in `partner-terminals.json` if the terminal exposes its own tab id.
...
**A partner's tab shows `ParserError: Missing expression after unary operator '--'`.** That tab's agent CLI has exited, and each wake-up line is being typed into the shell underneath it. The parse error is the shell rejecting the line, and it is harmless. From 0.23.0, after `UNANSWERED_NUDGES` (3) wake-ups the agent never acts on, the typing stops. Everyone then gets one system message naming the fix, and the dead agent no longer blocks the idle pause. Run `restart --id <id>` to relaunch it in a new tab, re-briefed from the transcript, then close the old tab.

**A partner's CLI died with nothing in its log.** Look for something that killed processes by pattern. A model once stopped a stray `wait` by killing every process matching `*partner.py*wait*`. Codex partners' launch prompts contained both words, so it took down partners in this repo and in another worktree. Since 0.23.0 the launch prompt no longer says `partner.py`, and every briefing gives `unwait`. That command stops exactly the calling agent's own `wait`, using the PID in its marker, and first checks that the PID is still a `partner.py wait`.

**A sandboxed partner cannot wake anyone ("Access is denied", "Could not read Orca runtime metadata").** Its sandbox blocks the terminal's CLI; Codex under `-s workspace-write` is the known case. Its nudges go through the session's waker instead. Check that `.partner/waker.json` has a heartbeat under 10s old. If it doesn't, any `wait` run by an agent that can drive the terminal starts one, for example a Claude Code session. A waker started from inside the sandbox exits at once by design.

**The session agent ended its turn with no `wait`, and a partner kept looping.** Before 0.22.0, the Stop hook counted any command in the last 15s as "a `wait` is starting", so a `send` just before the end of a turn let the agent go deaf. The hook now looks for the `wait` marker itself, for up to 5s.
