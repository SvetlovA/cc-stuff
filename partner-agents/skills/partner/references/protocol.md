...
**A CLI that has exited is left alone.** A wake-up only works if something in the tab reads it. When the agent's CLI is gone (killed, crashed, or quit), a shell reads it and rejects it, over and over. `<id>/.unanswered` counts wake-ups typed since the agent last acted, and any command it runs resets the count by moving `lastseen`. At `UNANSWERED_NUDGES` (3), `nudge_agent` stops typing and posts one system message to `@all` naming `restart --id <id>`. `agent_busy` then counts that agent as finished, so a dead partner does not keep everyone else looping. Terminal metadata cannot decide this: Orca still reports `agentIdentity: codex` for a tab whose Codex has exited. So the evidence is behavioral and works in any terminal.

**Stopping your own `wait`.** `unwait` kills the one process named in the caller's `<id>/waiting` marker, and only after its command line proves it is a Python `partner.py wait`, since PIDs are reused. It never matches by name. Partner CLIs carry their launch prompt on their command line, so the launch prompt avoids the literal `partner.py`.
...
3. **Every agent can be woken by a message** (`wakeable`). Either its tab can be typed into (`typable`: its terminal entry has a `send` template and the tab is on record), or it sleeps through the pause (`sleeps_through_pause`: a session agent that is not typable, whose background `wait` can sleep for free). A pause that strands a tab nobody can type into saves tokens by losing the partner, so it does not happen.

   The session agent's tab is on record the same way a spawned tab's is. `spawn` records what `orca terminal create` returns. The agent that ran `init` records its own tab from `self_handle` in its terminal's entry (`ORCA_TERMINAL_HANDLE` for Orca), at `init`, at `resume`, and whenever it runs `wait`. So inside Orca, the agent that started the session pauses and wakes exactly like the ones it spawned, whichever CLI it is.
...
- a **tab agent's** `wait` returns at once with the pause banner ("do not run `wait` again, end your turn"), and its briefing makes that the one legitimate exit from the loop besides a stop;
- an agent that **sleeps through the pause** keeps polling without returning, whatever its deadline. It is a background process of its harness, so sleeping costs nothing, and it is how that session hears a resume that starts in a tab. It also skips the pause notice itself: returning on it would re-invoke the model just to be told to sleep;
- `silent_agents` returns nobody, so no `wait`, `read` or `send` nudges a paused tab back one at a time;
- the Stop hook lets any paused agent end its turn. Only one that sleeps through the pause is still held to its one background `wait`;
- `liveness` reports a quiet agent as `paused` rather than `stale`.

**Resuming: the first message wakes everyone.** There is no command for it. `mark_active` runs for every message or instruction: `send` (so `kickoff` too), `claim`, `baton --to`, `spawn`, and `hook-prompt` when the human writes to a Claude agent. The boot prompt and wake-up lines disown themselves in their first words and are ignored. It stamps `activity` and, if `paused.json` existed, unlinks it; whichever process succeeds does the rest. That process posts a "session resumed" notice and nudges every running agent with no `wait` in flight, the session agent included. It skips an agent that started under 30s ago, because that tab is still booting. The nudge is unthrottled, so a nudge a minute earlier cannot swallow it, and it stamps `.nudge` afterwards so `send`'s own pass does not type a second line. The notice ends the sleeping `wait` of any agent that kept one. A human writing into a tab whose CLI has no prompt hook reaches the pause through that agent's briefing: `claim` first, which resumes everyone.
...
- **A pause lets every agent go.** While `paused.json` exists, check 2 does not apply, since leaving the loop is what a pause asks of it. Only an agent that sleeps through the pause (a session agent whose tab cannot be typed into) is still held to its background `wait`, which costs nothing.
