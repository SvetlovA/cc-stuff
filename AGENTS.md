# cc-stuff — Codex Plugin Marketplace

This repository is a public collection of Codex plugins. Each plugin lives in its own top-level directory and can be installed independently.
...
`.Codex-plugin/marketplace.json` at the repo root is what makes this repo installable as a marketplace. Codex reads it when a user installs from the GitHub URL.
...
2. Add `.Codex-plugin/plugin.json` inside it with at minimum `name`, `version`, and `description`
...
5. Add an entry to `.Codex-plugin/marketplace.json` at the repo root
